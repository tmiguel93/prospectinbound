...
import { leadScope, saleScope } from '../auth/auth.access.js';
...
salesRouter.post('/', async (req, res) => {
  ...
  if (lead.productId !== x.productId)
    return res
      .status(400)
      .json({ message: 'O produto da venda deve corresponder ao produto do lead.' });
  ...
  try {
    payment = await prisma.$transaction(async (tx) => {
      const firstPayment = await tx.payment.findFirst({
        where: { saleId: sale.id, isFirst: true },
        select: { id: true }
      });
      const isFirst = !firstPayment;
      ...
      if (rule) {
        ...
        await tx.commissionEntry.create({
          data: {
            paymentId: p.id,
            saleId: sale.id,
            type: isFirst ? 'ACTIVATION_AND_RECURRING' : 'RECURRING',
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002')
      return res.status(409).json({ message: 'Outro pagamento inicial foi registrado agora.' });
    throw error;
  }
...
salesRouter.patch('/:id', requireAdmin, async (req, res) => {
  const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
